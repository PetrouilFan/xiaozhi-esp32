...
private:
    ...
    struct context_t {
        ...
        size_t packet_len = 0;          // 缓冲区中累计的数据Length
        size_t seg_count = 0;           // 当前页段数
        size_t seg_index = 0;           // 当前Processing的段索引
        size_t data_offset = 0;         // 解析当前阶段AlreadyRead的字节数
        size_t bytes_needed = 0;        // 解析当前字段还需要Read的字节数
        size_t seg_remaining = 0;       // 当前段剩余需要Read的字节数
        size_t body_size = 0;           // 数据体总Size
        size_t body_offset = 0;         // 数据体AlreadyRead的字节数
    };
...
public:
    ...
    /// @brief Settings解封装完毕后回调ProcessingFunctions
