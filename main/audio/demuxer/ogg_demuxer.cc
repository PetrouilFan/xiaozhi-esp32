...
/// @param data 输入数据
/// @param size 输入数据Size
/// @return AlreadyProcessing的字节数
size_t OggDemuxer::Process(const uint8_t* data, size_t size)
{
    size_t processed = 0;  // AlreadyProcessing的字节数
        ...
        switch (state_) {
            ...
            if (ctx_.bytes_needed < 4) {
                ...
                if (ctx_.bytes_needed == 0) {
                    ...
                    if (memcmp(ctx_.header, "OggS", 4) == 0) {
                        ...
                        ctx_.data_offset = 4;
                        ctx_.bytes_needed = 27 - 4;  // 还需要23字节Done页头
                    } else {
                        // 匹配Failed，滑动1字节继续匹配
                ...
                if (found) {
                    ...
                    if (partial_len > 0) {
                        ...
                        processed += i;  // AlreadySearch所有字节
                    }
                    return processed;  // ReturnAlreadyProcessing的字节数
                }
            } else {
          ...
          case ParseState::PARSE_HEADER: {
            ...
            if (available < ctx_.bytes_needed) {
                ...
                // Verify页头
                if (ctx_.header[4] != 0) {
                    ESP_LOGE(TAG, "无效的OggVersion: %d", ctx_.header[4]);
                    state_ = ParseState::FIND_PAGE;
                    ctx_.bytes_needed = 4;
                    ctx_.data_offset = 0;
                    break;
                ...
                if (!seg_continued) {
                    ...
                    if (ctx_.packet_len) {
                        ...
                        if (opus_info_.head_seen && opus_info_.tags_seen) {
                            ...
                            ESP_LOGW(TAG, "当前Ogg容器Not yet解析到OpusHead/OpusTags，丢弃");
                        }
                    }
                    ctx_.packet_len = 0;
                    ctx_.packet_continued = false;
