...
private:
    ...
    void InitializeSDcardSpi() {
        ...
        if (err != ESP_OK) {
            ESP_LOGE(TAG, "SPI总线InitializeFailed: %s", esp_err_to_name(err));
            return;
        }
        ...
        if (err != ESP_OK) {
            ESP_LOGE(TAG, "SD卡挂载Failed: %s", esp_err_to_name(err));
            is_sdcard_found = false;
            return;
        } else if (err == ESP_OK) {
            ESP_LOGI(TAG, "SD卡挂载Success");
            is_sdcard_found = true;
        }
        // sdmmc_card_print_info(stdout, card); // PrintSD卡Info
    }
    ...
    void InitializePhysicalButtons() {
        ...
        boot_button_.OnMultipleClick(
            [this]() {
                ...
                this->TriggerVibrateEvent();
                if (is_sdcard_found) {
                    display_->SetChatMessage("system", "开机Detect到SD挂载Success");
                } else {
                    display_->SetChatMessage("system", "开机Detect到SD挂载Failed");
                }
            }, 5);
    ...
    void InitializeVibrateTask() {
        ...
        if (vibrate_event_queue_ == nullptr) {
            ESP_LOGE(TAG, "Create振动Event队列Failed");
            return;
        }
        ...
        if (ret != pdPASS) {
            ESP_LOGE(TAG, "Create振动任务Failed");
        } else {
            ESP_LOGI(TAG, "振动任务InitializeSuccess");
        }
    }
