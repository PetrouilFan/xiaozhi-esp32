...
private:
    ...
    static void ActionTask(void* arg) {
        ...
        while (true) {
            if (xQueueReceive(controller->action_queue_, &params, pdMS_TO_TICKS(1000)) == pdTRUE) {
                ...
                PowerManager::PauseBatteryUpdate();  // Action开始时暂停Battery levelUpdate
                controller->is_action_in_progress_ = true;
                if (params.action_type == ACTION_SERVO_SEQUENCE) {
                    // 执行舵机序列（自编程）- 仅支持短键名Format
                    cJSON* json = cJSON_Parse(params.servo_sequence_json);
                    if (json != nullptr) {
                        ESP_LOGD(TAG, "JSON解析Success，Length=%d", strlen(params.servo_sequence_json));
                        // 使用短键名 "a" 表示Action数组
                        cJSON* actions = cJSON_GetObjectItem(json, "a");
                        if (cJSON_IsArray(actions)) {
                            int array_size = cJSON_GetArraySize(actions);
                            ESP_LOGI(TAG, "执行舵机序列，共%d个Action", array_size);
                            
                            // Get序列执行Done后的Latency（短键名 "d"，顶层Parameters）
                            ...
                            // Initialize当前舵机Position（用于保持Not yet指定的舵机Position）
                            int current_positions[SERVO_COUNT];
                            for (int j = 0; j < SERVO_COUNT; j++) {
                                current_positions[j] = 90;  // 默认中间Position
                            }
                            // 手部舵机默认Position
                            ...
                            for (int i = 0; i < array_size; i++) {
                                ...
                                if (cJSON_IsObject(action_item)) {
                                    ...
                                    if (cJSON_IsObject(osc_item)) {
                                        ...
                                        // Read中心Angle（短键名 "o"），默认90度（绝对Angle0-180度）
                                        for (int j = 0; j < SERVO_COUNT; j++) {
                                            center_angle[j] = 90;  // 默认中心Angle90度（中间Position）
                                        }
                                        ...
                                        // 安全Check：防止左右腿脚同时做大幅度振荡（振幅Check）
                                        ...
                                        if (left_leg_large && right_leg_large) {
                                            ESP_LOGW(TAG, "Detect到左右腿同时大幅度振荡，限制右腿振幅");
                                            amplitude[RIGHT_LEG] = 0;  // Forbid右腿振荡
                                        }
                                        if (left_foot_large && right_foot_large) {
                                            ESP_LOGW(TAG, "Detect到左右脚同时大幅度振荡，限制右脚振幅");
                                            amplitude[RIGHT_FOOT] = 0;  // Forbid右脚振荡
                                        }
                                        
                                        // Read相位差（短键名 "ph"，单位为度，转换为弧度）
                                        ...
                                        // ReadPeriod（短键名 "p"），范围100-3000毫Second
                                        cJSON* period_item = cJSON_GetObjectItem(osc_item, "p");
                                        if (cJSON_IsNumber(period_item)) {
                                            period = period_item->valueint;
                                            if (period < 100) period = 100;
                                            if (period > 3000) period = 3000;  // 与描述一致，限制3000毫Second
                                        }
                                        
                                        // ReadPeriod数（短键名 "c"），范围0.1-20.0
                                        ...
                                        // 执行振荡 - 使用Execute2，以绝对Angle为中心
                                        ESP_LOGI(TAG, "执行振荡Action%d: period=%d, steps=%.1f", i, period, steps);
                                        controller->otto_.Execute2(amplitude, center_angle, period, phase_diff, steps);
                                        
                                        // 振荡后UpdatePosition（使用center_angle作为最终Position）
                                        for (int j = 0; j < SERVO_COUNT; j++) {
                                            current_positions[j] = center_angle[j];
                                        }
                                    } else {
                                        // 普通MoveMode
                                        // 从当前Position数组Copy，保持Not yet指定的舵机Position
                                        int servo_target[SERVO_COUNT];
                                        for (int j = 0; j < SERVO_COUNT; j++) {
                                            servo_target[j] = current_positions[j];
                                        }
                                        
                                        // 从JSON中Read舵机Position（短键名 "s"）
                                        ...
                                        // GetMoveSpeed（短键名 "v"，默认1000毫Second）
                                        int speed = 1000;
                                        cJSON* speed_item = cJSON_GetObjectItem(action_item, "v");
                                        if (cJSON_IsNumber(speed_item)) {
                                            speed = speed_item->valueint;
                                            if (speed < 100) speed = 100;  // 最小100毫Second
                                            if (speed > 3000) speed = 3000;  // 最大3000毫Second
                                        }
                                        
                                        // 执行舵机Move
                                        ESP_LOGI(TAG, "执行Action%d: ll=%d, rl=%d, lf=%d, rf=%d, v=%d",
                                        ...
                                        // Update当前Position数组，用于下一个Action
                                        for (int j = 0; j < SERVO_COUNT; j++) {
                                            current_positions[j] = servo_target[j];
                                        }
                                    }
                                    
                                    // GetAction后的LatencyTime（短键名 "d"）
                                    int delay_after = 0;
                                    ...
                                    // Action后的Latency（最后一个Action后不Latency）
                                    if (delay_after > 0 && i < array_size - 1) {
                                        ESP_LOGI(TAG, "Action%d执行Done，Latency%d毫Second", i, delay_after);
                                        vTaskDelay(pdMS_TO_TICKS(delay_after));
                                    }
                                }
                            }
                            
                            // 序列执行Done后的Latency（用于序列之间的停顿）
                            if (sequence_delay > 0) {
                                // Check队列中是否还有待执行的序列
                                UBaseType_t queue_count = uxQueueMessagesWaiting(controller->action_queue_);
                                if (queue_count > 0) {
                                    ESP_LOGI(TAG, "序列执行Done，Latency%d毫Second后执行下一个序列（队列中还有%d个序列）", 
                            ...
                            ESP_LOGE(TAG, "舵机序列FormatError: 'a'不是数组");
                            cJSON_Delete(json);
                        }
                    } else {
                        // GetcJSON的ErrorInfo
                        ...
                        ESP_LOGE(TAG, "解析舵机序列JSONFailed，Length=%d，ErrorPosition: %s", json_len, 
                                 error_ptr ? error_ptr : "Not yet知");
                ...
                PowerManager::ResumeBatteryUpdate();  // Action结束时恢复Battery levelUpdate
    ...
    void QueueServoSequence(const char* servo_sequence_json) {
        ...
        const int buffer_size = 512;  // servo_sequence_json数组Size
        ESP_LOGI(TAG, "队列舵机序列，输入Length=%d，缓冲区Size=%d", input_len, buffer_size);
        
        if (input_len >= buffer_size) {
            ESP_LOGE(TAG, "JSON字符串太长！输入Length=%d，最大Allow=%d", input_len, buffer_size - 1);
            return;
        }
        ...
        ESP_LOGD(TAG, "序列AlreadyJoin队列: %s", params.servo_sequence_json);
...
public:
    OttoController(const HardwareConfig& hw_config) {
        ...
        ESP_LOGI(TAG, "Otto机器人Initialize%s手部舵机", has_hands_ ? "带" : "不带");
        ESP_LOGI(TAG, "舵机引脚Configuration: LL=%d, RL=%d, LF=%d, RF=%d, LH=%d, RH=%d",
    ...
    void RegisterMcpTools() {
        ...
        // 统一Action工具（除了舵机序列外的所有Action）
        mcp_server.AddTool("self.otto.action",
                           "执行机器人Action。action: Action名称；根据ActionClass型提供相应Parameters：direction: Direction，1=前进/左转，-1=后退/右转；0=左右同时"
                           "steps: Action步数，1-100；speed: ActionSpeed，100-3000，数值越小越快；amount: Action幅度，0-170；arm_swing: 手臂摆动幅度，0-170；"
                           "基础Action：walk(行走，需steps/speed/direction/arm_swing)、turn(转身，需steps/speed/direction/arm_swing)、jump(跳跃，需steps/speed)、"
                           ...
                           "固定Action：sit(坐下)、showcase(展示Action)、home(Reset)；"
                           "手部Action(需手部舵机)：hands_up(举手，需speed/direction)、hands_down(放手，需speed/direction)、hand_wave(挥手，需direction)、"
                           ...
                           [this](const PropertyList& properties) -> ReturnValue {
                               ...
                               // 所有Parameters都有Default value，直接访问即可
                               ...
                               // 手部Action
                                   ...
                                   return "Error：无效的Action名称。可用Action：walk, turn, jump, swing, moonwalk, bend, shake_leg, updown, whirlwind_leg, sit, showcase, home, hands_up, hands_down, hand_wave, windmill, takeoff, fitness, greeting, shy, radio_calisthenics, magic_circle";
                               }
                           });


        // 舵机序列工具（支持分段Send，每次Send一个序列，自动排队执行）
        mcp_server.AddTool(
            "self.otto.servo_sequences",
            "AI自DefineAction编程（即兴Action）。支持分段Send序列：超过5个序列SuggestionAI可以连续多次Call此工具，每次Send一个短序列，系统会自动排队按顺序执行。支持普通Move和振荡器两种Mode。"
            ...
            "sequence: 单个序列对象，Include'a'Action数组，顶层可选Parameters："
            "'d'(序列执行Done后Latency毫Second数，用于序列之间的停顿)。"
            "每个Action对象Include："
            "普通Mode：'s'舵机Position对象(键名：ll/rl/lf/rf/lh/rh，值：0-180度)，'v'MoveSpeed100-3000毫Second(默认1000)，'d'Action后Latency毫Second数(默认0)；"
            "振荡Mode：'osc'振荡器对象，Include'a'振幅对象(各舵机振幅10-90度，默认20度)，'o'中心Angle对象(各舵机振荡中心绝对Angle0-180度，默认90度)，'ph'相位差对象(各舵机相位差，度，0-360度，默认0度)，'p'Period100-3000毫Second(默认500)，'c'Period数0.1-20.0(默认5.0)；"
            "使用方式：AI可以连续多次Call此工具，每次Send一个序列，系统会自动排队按顺序执行。"
            "重要Description：左右腿脚震荡的时候，有一只脚必须在90度，否则会损坏机器人，如果Send多个序列（序列数>1），Done所有序列后需要Reset时，AI应该最后单独Callself.otto.home工具进行Reset，不要在序列中SettingsResetParameters。"
            "普通ModeExample：Send3个序列，最后CallReset："
            "第1次Call{\"sequence\":\"{\\\"a\\\":[{\\\"s\\\":{\\\"ll\\\":100},\\\"v\\\":1000}],\\\"d\\\":500}\"}，"
            "第2次Call{\"sequence\":\"{\\\"a\\\":[{\\\"s\\\":{\\\"ll\\\":90},\\\"v\\\":800}],\\\"d\\\":500}\"}，"
            "第3次Call{\"sequence\":\"{\\\"a\\\":[{\\\"s\\\":{\\\"ll\\\":80},\\\"v\\\":800}]}\"}，"
            "最后Callself.otto.home工具进行Reset。"
            "振荡器ModeExample："
            "Example1-双臂Sync摆动：{\"sequence\":\"{\\\"a\\\":[{\\\"osc\\\":{\\\"a\\\":{\\\"lh\\\":30,\\\"rh\\\":30},\\\"o\\\":{\\\"lh\\\":90,\\\"rh\\\":-90},\\\"p\\\":500,\\\"c\\\":5.0}}],\\\"d\\\":0}\"}；"
            "Example2-双腿交替振荡（波浪效果）：{\"sequence\":\"{\\\"a\\\":[{\\\"osc\\\":{\\\"a\\\":{\\\"ll\\\":20,\\\"rl\\\":20},\\\"o\\\":{\\\"ll\\\":90,\\\"rl\\\":-90},\\\"ph\\\":{\\\"rl\\\":180},\\\"p\\\":600,\\\"c\\\":3.0}}],\\\"d\\\":0}\"}；"
            "Example3-单腿振荡配合固定脚（安全）：{\"sequence\":\"{\\\"a\\\":[{\\\"osc\\\":{\\\"a\\\":{\\\"ll\\\":45},\\\"o\\\":{\\\"ll\\\":90,\\\"lf\\\":90},\\\"p\\\":400,\\\"c\\\":4.0}}],\\\"d\\\":0}\"}；"
            "Example4-复杂多舵机振荡（手和腿）：{\"sequence\":\"{\\\"a\\\":[{\\\"osc\\\":{\\\"a\\\":{\\\"lh\\\":25,\\\"rh\\\":25,\\\"ll\\\":15},\\\"o\\\":{\\\"lh\\\":90,\\\"rh\\\":90,\\\"ll\\\":90,\\\"lf\\\":90},\\\"ph\\\":{\\\"rh\\\":180},\\\"p\\\":800,\\\"c\\\":6.0}}],\\\"d\\\":500}\"}；"
            "Example5-快速摇摆：{\"sequence\":\"{\\\"a\\\":[{\\\"osc\\\":{\\\"a\\\":{\\\"ll\\\":30,\\\"rl\\\":30},\\\"o\\\":{\\\"ll\\\":90,\\\"rl\\\":90},\\\"ph\\\":{\\\"rl\\\":180},\\\"p\\\":300,\\\"c\\\":10.0}}],\\\"d\\\":0}\"}。",
            ...
            [this](const PropertyList& properties) -> ReturnValue {
                ...
                // Check是否是JSON对象（可能是字符串Format或Already解析的对象）
        ...
        mcp_server.AddTool("self.otto.stop", "ImmediatelyStop所有Action并Reset", PropertyList(),
                           [this](const PropertyList& properties) -> ReturnValue {
                               if (action_task_handle_ != nullptr) {
                               ...
                               PowerManager::ResumeBatteryUpdate();  // StopAction时恢复Battery levelUpdate
        ...
        mcp_server.AddTool(
            "self.otto.set_trim",
            "校准单个舵机Position。Settings指定舵机的微调Parameters以调整机器人的初始站立姿态，Settings将永久Save。"
        ...
        mcp_server.AddTool("self.otto.get_trims", "Get当前的舵机微调Settings", PropertyList(),
                           [this](const PropertyList& properties) -> ReturnValue {
                               Settings settings("otto_trims", false);
        ...
        mcp_server.AddTool("self.otto.get_status", "Get机器人Status，Return moving 或 idle",
        ...
        mcp_server.AddTool("self.otto.get_ip", "Get机器人WiFi IP地址", PropertyList(),
                           [](const PropertyList& properties) -> ReturnValue {
...
void InitializeOttoController(const HardwareConfig& hw_config) {
    if (g_otto_controller == nullptr) {
        ...
        ESP_LOGI(TAG, "Otto控制器AlreadyInitialize并RegisterMCP工具");
    }
}
