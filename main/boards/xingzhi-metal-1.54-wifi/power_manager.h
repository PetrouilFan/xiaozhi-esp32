...
private:
    ...
    // power
    bool pressed = false;                   // 是否按下Power键
    int PowerControl_ticks_ = 0;            // 开机时长
    int press_ticks_ = 0;                   // 按下时的tick
    bool is_first_boot = true;              // 
    uint8_t PowerDec_level_ = 0;            // Power键电平
    const int power_off_ticks_ = 20;        // 按键按下20/5SecondPrepare关机;
    bool new_charging_status = false;       // 插入usb时Cannot软件关机
    bool is_shutting_down_ = false;  // 标记是否Enter关机流程
    int shutdown_delay_ticks_ = 0;   // 关机Latency计数
    uint8_t shutdown_ticks = 5;      // 5/5s后关机 预留的关机Operation
...
public:
    PowerManager(gpio_num_t pin) : charging_pin_(pin) {
        ...
        ESP_ERROR_CHECK(adc_oneshot_config_channel(adc_handle_, POWER_BATTERY_ADC_CHANNEL, &chan_config)); // BatteryBattery level
    ...
    void shutdown() {
        ...
        {
            shutdown_first_ = false; // Enter后置 false ，防止再次Enter关机Status
            ...
            ESP_LOGI("PowerManager","关机Failed，Enter深Sleep");
            esp_deep_sleep_start();
        } else {
            ESP_LOGI("PowerManager","Detect到插入usb，Cannot关机"); 
        }
    }
};
