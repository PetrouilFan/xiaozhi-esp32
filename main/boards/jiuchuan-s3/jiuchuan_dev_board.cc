...
private:
    ...
    // 音量映射Functions：将内部音量(0-80)映射为显示音量(0-100%)
    ...
    void InitializePowerSaveTimer() {
        ...
        #endif
        //一MinuteEnter浅Sleep，5MinuteEnter深Sleep关机
        ...
        power_save_timer_->OnShutdownRequest([this]() {
            ...
            esp_lcd_panel_disp_on_off(panel, false); //Close显示
            esp_deep_sleep_start();
            #else
    ...
    void InitializeButtons() {
        ...
        ESP_LOGI(TAG, "Power button long press detected (high-active)");
            ...
            if (pwrbutton_unreleased){
                ESP_LOGI(TAG, "开机后Power键Not yet松开,Cancel关机");
                return;
            }
        ...
        pwr_button_.OnClick([this]()
            ...
            ESP_LOGI(TAG, "当前设备Status: %d", current_state);
            
            if (current_state == kDeviceStateIdle) {
                // 如果当前是待命Status，切换到聆听Status
                ESP_LOGI(TAG, "从待命Status切换到聆听Status");
                app.ToggleChatState(); // 切换到聆听Status
            } else if (current_state == kDeviceStateListening) {
                // 如果当前是聆听Status，切换到待命Status
                ESP_LOGI(TAG, "从聆听Status切换到待命Status");
                app.ToggleChatState(); // 切换到待命Status
            } else if (current_state == kDeviceStateSpeaking) {
                // 如果当前是说话Status，终止说话并切换到待命Status
                ESP_LOGI(TAG, "从说话Status切换到待命Status");
        ...
        wifi_button.OnPressDown([this]()
           ...
           ESP_LOGI(TAG, "Volume up button pressed");
            ...
            int current_vol = codec->output_volume(); // Get实际当前音量
