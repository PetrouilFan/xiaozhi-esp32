...
private:
    ...
    void InitializeButtons() {
        ...
        boot_button_.OnDoubleClick([this]() {
            ...
            if (is_first_boot) {
                ESP_LOGI(TAG, "First boot, enable double-tap photo");
                ...
                ESP_LOGI(TAG, "Non-first boot, disable double-tap photo");
                auto& app = Application::GetInstance();
    ...
    void InitializeCamera() {
        ...
        config.ledc_channel = LEDC_CHANNEL_2;  // LEDC channel select  用于生成XCLK时钟 但是S3不用
        config.ledc_timer = LEDC_TIMER_2; // LEDC timer select  用于生成XCLK时钟 但是S3不用
        ...
        config.pin_sccb_sda = -1;   // 这里写-1 表示使用Already经Initialize的I2C接口
