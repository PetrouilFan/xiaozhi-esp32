...
private:
    ...
    void InitializeTools()
    {
        ...
        // 基础Action控制
        mcp_server.AddTool("self.dog.basic_control", "机器人的基础Action。机器人可以做以下基础Action：\n"
            "forward: 向前Move\nbackward: 向后Move\nturn_left: 向左转\nturn_right: 向右转\nstop: ImmediatelyStop当前Action", 
            PropertyList({
        ...
        // 扩展Action控制
        mcp_server.AddTool("self.dog.advanced_control", "机器人的扩展Action。机器人可以做以下扩展Action：\n"
        ...
        // 灯光控制
        mcp_server.AddTool("self.light.get_power", "Get灯是否Open", PropertyList(), [this](const PropertyList& properties) -> ReturnValue {
            return led_on_;
        });

        mcp_server.AddTool("self.light.turn_on", "Open灯", PropertyList(), [this](const PropertyList& properties) -> ReturnValue {
            SetLedColor(0xFF, 0xFF, 0xFF);
            led_on_ = true;
            return true;
        });

        mcp_server.AddTool("self.light.turn_off", "Close灯", PropertyList(), [this](const PropertyList& properties) -> ReturnValue {
            SetLedColor(0x00, 0x00, 0x00);
            led_on_ = false;
            return true;
        });

        mcp_server.AddTool("self.light.set_rgb", "SettingsRGB颜色", PropertyList({
