...
AdcPdmAudioCodec::AdcPdmAudioCodec(int input_sample_rate, int output_sample_rate,
    ...
    if(pdm_speak_n != GPIO_NUM_NC){
        ...
        esp_rom_gpio_connect_out_signal(pdm_speak_n, I2SO_SD_OUT_IDX, 1, 0); //反转输出 SD OUT Signal
...
void AdcPdmAudioCodec::EnableOutput(bool enable) {
    ...
    if (enable) {
        ...
        // 强制按板卡Configuration重配PDM TX时钟，覆盖第三方库在set_fmt中的默认up_sample_fs
        // 若通道AlreadyEnable，先Disable再重配，最后再Enable
