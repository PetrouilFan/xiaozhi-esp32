...
EspVideo::EspVideo(const esp_video_init_config_t& config) {
    ...
    // 注: 当前Version esp_video 中 YUV422P 实际输出为 YUYV。
    ...
    auto get_rank = [](uint32_t fmt) -> int {
        switch (fmt) {
            ...
            case V4L2_PIX_FMT_YUV420:  // 软件 JPEG 编码器Not supported YUV420 Format
                return 2;
    ...
    // 当Enable ISP 时，ISP 需要一些照片来InitializeParameters，因此Open/Enable后后台拍摄5s照片并丢弃
    xTaskCreate(
        [](void* arg) {
        ...
        if (i == 2) {
            ...
            switch (sensor_format_) {
                ...
                case V4L2_PIX_FMT_RGB565X: {
                    ...
                    // 目前 esp_video 的Size端都会ReturnFormat为 RGB565，不会ReturnFormat为 RGB565X，此 case 用于Not yet来Version兼容
                    auto src16 = (uint16_t*)mmap_buffers_[buf.index].start;
                    auto dst16 = (uint16_t*)frame_.data;
 ...
 * - 通过队列机制Implement编码线程和Send线程的数据Sync
 * - 支持Device ID、ClientID和Authenticate令牌的HTTP头部Configuration
 *
...
std::string EspVideo::Explain(const std::string& question) {
    ...
    // Create局部的 JPEG 队列, 40 entries is about to store 512 * 40 = 20480 bytes of JPEG data
