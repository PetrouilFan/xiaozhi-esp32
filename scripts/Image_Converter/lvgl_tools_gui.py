...
HELP_TEXT = """LVGL图片转换工具使用Description：

1. 添加Files：点击“添加Files”按钮选择需要转换的图片，支持批量Import

2. 移除Files：在列表中选中Files前的复选框“[ ]”（选中后会变成“[√]”），点击“移除选中”可Delete选定Files

3. Settings分辨率：选择需要的分辨率，如128x128
   Suggestion根据自己的设备的屏幕分辨率来选择。过大和过小都会影响显示效果。

4. 颜色Format：选择“自动识别”会根据图片是否透明自动选择，或手动指定
   除非你了解这个Options，否则Suggestion使用自动识别，不然可能会出现一些意想不到的问题……

5. 压缩方式：选择NONE或RLE压缩
   除非你了解这个Options，否则Suggestion保持默认NONE不压缩

6. 输出Directory：Settings转换后Files的Save路径
   默认为程序所在Directory下的outputFiles夹
...
class ImageConverterApp:
    ...
    def create_widgets(self):
        # ParametersSettings框架
        settings_frame = ttk.LabelFrame(self.root, text="转换Settings")
        ...
        # 颜色Format
        ttk.Label(settings_frame, text="颜色Format:").grid(row=0, column=2, padx=2)
        ...
        # FilesOperation框架
        file_frame = ttk.LabelFrame(self.root, text="选取Files")
        file_frame.grid(row=1, column=0, padx=10, pady=5, sticky="nsew")

        # FilesOperation按钮
        btn_frame = ttk.Frame(file_frame)
        btn_frame.pack(fill=tk.X, pady=2)
        ttk.Button(btn_frame, text="添加Files", command=self.select_files).pack(side=tk.LEFT, padx=2)
        ttk.Button(btn_frame, text="移除选中", command=self.remove_selected).pack(side=tk.LEFT, padx=2)
        ttk.Button(btn_frame, text="Clear列表", command=self.clear_files).pack(side=tk.LEFT, padx=2)
        ...
        # 输出Directory
        output_frame = ttk.LabelFrame(self.root, text="输出Directory")
        ...
        ttk.Button(convert_frame, text="转换全部Files", command=lambda: self.start_conversion(True)).pack(side=tk.LEFT, padx=5)
        ttk.Button(convert_frame, text="转换选中Files", command=lambda: self.start_conversion(False)).pack(side=tk.LEFT, padx=5)
        ...
        # Clear日志按钮
        ttk.Button(log_btn_frame, text="Clear日志", command=self.clear_log).pack(side=tk.RIGHT, padx=5, pady=2)
    ...
    def select_files(self):
        files = filedialog.askopenfilenames(filetypes=[("图片Files", "*.png;*.jpg;*.jpeg;*.bmp;*.gif")])
        for f in files:
    ...
    def convert_images(self, input_files, width, height, compress):
        ...
        for idx, file_path in enumerate(input_files):
            try:
                print(f"CurrentlyProcessing: {os.path.basename(file_path)}")
                ...
                with Image.open(file_path) as img:
                    ...
                    success_count += 1
                    os.unlink(temp_path)
                    print(f"Success转换: {base_name}.c\n")

            except Exception as e:
                print(f"转换Failed: {str(e)}\n")

        print(f"转换Done! Success {success_count}/{total_files} 个Files\n")
